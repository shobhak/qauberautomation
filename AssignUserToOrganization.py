...
class AssignUserToOrgnization(unittest.TestCase):
    ...
    def test_AssignUser(self):
        ...
        driver.save_screenshot(
            'C:\Users\UNI\Desktop\QAUber\Automation Assignment\SCREENSHOTS\InviteAdminUser' + newtime + '.png')
        driver.find_element_by_xpath("//button[contains(@ng-click,'ctrl.nextStep()')]").click()
        time.sleep(2)
        ...
        time.sleep(2)
        Actual_OrgName = driver.find_element_by_xpath("//tr[98]/td[3]/div").text
        print (Actual_OrgName,"On Users Page")
        message = "Expected result to be {expected} and actual result is {actual} ".format(
            expected=cname,
            actual=Actual_OrgName
        )
        self.assertEqual(cname,Actual_OrgName,message)
        UserRight = driver.find_element_by_xpath("//tr[98]/td[1]/div").text
        self.assertEqual("Admin", UserRight)
        print (Actual_OrgName,"user right is" , UserRight)
